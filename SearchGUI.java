...
public class SearchGUI extends JFrame {
	...
	/** Directory of the index to load **/
	String pathIndexToLoad ;
	...
	public static final String homeDir = "/home/nightwish/Documents/kth/SearchEngines/ir-lab";
     ...
     */
    private void index() {
    	if (indexType == Index.LOADED_INDEX) {
    		loadIndex();
    	}
    	else if ( indexType == Index.HASHED_INDEX){
    		synchronized ( indexLock ) {
    ...
    private void loadIndex()  {
    	synchronized ( indexLock ) {
    		...
    		indexer.loadIndex(pathIndexToLoad);
    		resultWindow.setText( "\n  Done!" );
    	}
    };
    /**
     *   Decodes the command line arguments. 
     *   -l added to load an index
     */
    private void decodeArgs( String[] args ) {
    	...
    	while ( i < args.length ) {
    		if ( "-d".equals( args[i] )) {
    			...
    			i++;
    			if ( i < args.length ) {
    				indexType = Index.LOADED_INDEX;
    				pathIndexToLoad = args[i++] ;
    			}
    		}
    		else {
